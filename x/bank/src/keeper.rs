...
impl<SK: StoreKey, PSK: ParamsSubspaceKey> Keeper<SK, PSK> {
    ...
    fn send_coins<DB: Database>(
        ...
        for send_coin in msg.amount {
            ...
            // 这里在存储更新的时候，是以 coin type作为key， 以对应的amount作为value的
            // 因为bank可能有多种denom，所以需要标识起来（比如uatom,2），那么实际存储的时候，完整的key是一个拼接：<bank-uatom>, value是2
            ...
            // 比如是uatom，那么此时get的时候就是从bank-utom这个key来获取对应账户的value
