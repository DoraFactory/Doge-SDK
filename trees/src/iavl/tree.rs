...
// i-avl树对应一个区块，一个区块号也就是对应一个version，所以用version来管理i-avl树的版本（因为avl树是不可变的）
// 树有两个节点组成，一个是InnderNode(非叶子节点)，二是LeafNode(叶子节点)
...
impl InnerNode {
    ...
    pub fn get_left_node<T: Database>(&self, node_db: &NodeDB<T>) -> Arc<Node> {
    ...
    pub fn get_right_node<T: Database>(&self, node_db: &NodeDB<T>) -> Arc<Node> {
...
// 叶子节点是实际存储value(实际的一些应用数据，比如余额)的节点，它和非叶子节点不同
#[derive(Debug, Clone, PartialEq, Default)]
pub struct LeafNode {
    key: Vec<u8>,
    value: Vec<u8>,
    ...
    fn hash_serialize(&self) -> Vec<u8> {
        match &self {
            Node::Leaf(node) => {
                ...
                let height: i64 = 0;
                let size: i64 = 1;
                let version: i64 = (node.version as u32).into();
...
where
    T: Database,
{
    pub fn new(db: T, target_version: Option<u64>) -> Result<Tree<T>, Error> {
        let node_db = NodeDB::new(db);
        // 获取所有的区块号，一开始初始化的时候是空的BTreeSet
    ...
    pub fn update(&mut self, node: Option<Arc<Node>>, updated_version: u64) {
        self.root = node;
        self.loaded_version = updated_version;
        self.versions.insert(updated_version);
    }
    ...
    pub fn root_node(&mut self, version: u64) -> Option<Arc<Node>> {
        let node_db = &mut self.node_db;
        node_db.get_root_node(version).ok()?.map(|node| Arc::new(node))
    }
    ...
    fn right_rotate(node: &mut Node, version: u64, node_db: &NodeDB<T>) -> Result<(), Error> {
    ...
    fn left_rotate(node: &mut Node, version: u64, node_db: &NodeDB<T>) -> Result<(), Error> {
        if let Node::Inner(z) = node {
            let mut z = mem::take(z);
