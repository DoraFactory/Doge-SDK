...
impl<T: Database> MultiStore<T> {
    pub fn new(db: T) -> Self {
        let db = Arc::new(db);
        // 通过使用K-V数据库来存储某一个module中的数据
        // 比如bank模块，首先创建一个bank的kv数据库，以bank这个前缀来作为一个存储的
...
impl<T: Database> KVStore<T> {
    pub fn new(db: T, target_version: Option<u64>) -> Result<Self, AppError> {
        Ok(KVStore {
